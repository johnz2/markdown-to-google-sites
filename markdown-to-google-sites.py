# https://github.com/johnz2/markdown-to-google-sites
...
def markdown_to_google_sites(input_filename, print_markdown_html=False,
    ...
    markdown_to_gsites_notice = "<!-- Autogenerated by \
    ...
    for filename in glob.glob(input_filename):
        ...
        # markdown -> google sites html
        html = read_file_into_html(filename)
    
        if print_markdown_html:
            script_print(html, show_banner=False, is_html_output=True)

        save_name = path.splitext(filename)[0] + '.html'
        
        save_file = codecs.open(save_name, mode='w', encoding='utf-8')
        save_file.write(html)
        save_file.close()
        
        # 
        script_print("wrote {0}".format(save_name))
        script_print("done!")
